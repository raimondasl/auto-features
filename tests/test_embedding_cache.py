"""Tests for anonymous.embedding_cache — the persistent per-paper embedding cache."""
...
from anonymous.embedding_cache import (
    default_model,
    embed_papers_cached,
    load_cached_vectors,
    vec_from_bytes,
...
class TestEmbedPapersCached:
    def test_computes_only_uncached_then_reuses(self, tmp_path: Path) -> None:
        with PaperStore(tmp_path / "papers.db") as store:
            ...
            with patch(
                "anonymous.embeddings.compute_paper_embedding", side_effect=_fake_paper_emb
            ) as m:
    ...
    def test_load_cached_vectors(self, tmp_path: Path) -> None:
        with PaperStore(tmp_path / "papers.db") as store:
            store.upsert_paper(_paper("a"))
            with patch("anonymous.embeddings.compute_paper_embedding", side_effect=_fake_paper_emb):
                embed_papers_cached(store, [_paper("a")])
