...
class TestSendDesktopNotification:
    @patch("anonymous.watcher.platform.system", return_value="Linux")
    @patch("anonymous.watcher.subprocess.run")
    ...
    @patch("anonymous.watcher.platform.system", return_value="Darwin")
    @patch("anonymous.watcher.subprocess.run")
    ...
    @patch("anonymous.watcher.platform.system", return_value="Windows")
    @patch("anonymous.watcher.subprocess.run")
    ...
    @patch("anonymous.watcher.platform.system", return_value="FreeBSD")
    def test_unsupported_platform(self, mock_sys: MagicMock) -> None:
        assert send_desktop_notification("Title", "Msg") is False


class TestRunUpdateCycle:
    @patch("anonymous.pipeline.PaperStore")
    @patch("anonymous.pipeline.collect_papers")
    @patch("anonymous.pipeline.build_queries", return_value=["all:test"])
    @patch("anonymous.pipeline.profile_repo")
    @patch("anonymous.pipeline.rank_papers")
    @patch("anonymous.digest.write_digest")
    @patch("anonymous.config.load_config")
    @patch("anonymous.config.validate_config", return_value=[])
    def test_successful_cycle(
        ...
        cfg = AnonymousConfig(repo_path="/tmp/repo")
    ...
    def test_a_broken_disclosure_does_not_stop_the_cycle(
        ...
        with patch("anonymous.stages.unrun_stages", side_effect=RuntimeError("registry exploded")):
            result = run_update_cycle("/tmp/config.yml")
    ...
    @patch("anonymous.pipeline.collect_papers")
    @patch("anonymous.pipeline.build_queries", return_value=["q"])
    @patch("anonymous.pipeline.profile_repo")
    @patch("anonymous.config.load_config")
    @patch("anonymous.config.validate_config", return_value=[])
    ...
    @patch("anonymous.pipeline.collect_papers")
    @patch("anonymous.pipeline.build_queries", return_value=[])
    @patch("anonymous.pipeline.profile_repo")
    @patch("anonymous.config.load_config")
    @patch("anonymous.config.validate_config", return_value=[])
    def test_no_queries(
...
class TestWatchLoop:
    @patch("anonymous.watcher.time.sleep")
    @patch("anonymous.watcher.run_update_cycle")
    ...
    @patch("anonymous.watcher.send_desktop_notification")
    @patch("anonymous.watcher.time.sleep")
    @patch("anonymous.watcher.run_update_cycle")
...
class TestWatcherIntegrity:
    ...
    def _run(self, tmp_path, comments: dict[str, str]):
        ...
        from anonymous.config import AnonymousConfig, SignalsConfig
        from anonymous.store import PaperStore

        repo = Path(tmp_path)
        repo.mkdir(parents=True, exist_ok=True)
        (repo / "README.md").write_text("retrieval augmented generation", encoding="utf-8")
        cfg = AnonymousConfig(repo_path=str(repo), signals=SignalsConfig(integrity=True))
        ...
        with (
            patch("anonymous.config.load_config", return_value=cfg),
            patch("anonymous.config.validate_config", return_value=[]),
            patch("anonymous.pipeline.collect_papers", return_value=[paper]),
            patch("anonymous.signals.integrity.fetch_comments", return_value=comments),
            patch("anonymous.digest.write_digest", return_value=(repo / "d.md", None)),
        ):
