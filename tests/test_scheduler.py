...
from anonymous.scheduler import (
    CRON_MARKER,
    _build_command,
...
class TestCronJobs:
    @patch("anonymous.scheduler._set_crontab", return_value=True)
    @patch("anonymous.scheduler._get_current_crontab", return_value="")
    ...
    @patch("anonymous.scheduler._set_crontab", return_value=True)
    @patch(
        "anonymous.scheduler._get_current_crontab",
...
class TestSchtasks:
    @patch("anonymous.scheduler.subprocess.run")
...
class TestPublicInterface:
    @patch("anonymous.scheduler._detect_platform", return_value="unix")
    @patch("anonymous.scheduler.add_cron_job", return_value=True)
    ...
    @patch("anonymous.scheduler._detect_platform", return_value="windows")
    @patch("anonymous.scheduler.add_schtask", return_value=True)
    ...
    @patch("anonymous.scheduler._detect_platform", return_value="unix")
    @patch("anonymous.scheduler.list_cron_jobs", return_value=[])
    ...
    @patch("anonymous.scheduler._detect_platform", return_value="windows")
    @patch("anonymous.scheduler.list_schtasks", return_value=[])
    ...
    @patch("anonymous.scheduler._detect_platform", return_value="unix")
    @patch("anonymous.scheduler.remove_cron_job", return_value=True)
    def test_remove_schedule_unix(self, mock_rm: MagicMock, mock_plat: MagicMock) -> None:
        assert remove_schedule() is True
        mock_rm.assert_called_once()

    @patch("anonymous.scheduler._detect_platform", return_value="windows")
    @patch("anonymous.scheduler.remove_schtask", return_value=True)
