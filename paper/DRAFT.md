...
6. **A catalogue of 33 negative results and 12 corrections** (§10), and a taxonomy of the silent harness failures that repeatedly manufactured wrong findings, with the defensive patterns that stopped them (§11).
...
- **[C-12]** The eval harness merged non-arXiv sources on raw arXiv identifiers while the product version-strips them, so `2605.23815v1` and `2605.23815` both survived: 6 duplicate papers across 4 cases in the S2 treatment arm, none in the control. Per-case values were contaminated; the corrections cancel exactly at the mean (§9.5).
