...
| **+5.72** | **25** | **15** | 50 | frozen | 08-16 | **the headline**, at `w_embedding: 1.5` — and the first row with a baseline on all 25 |
...
| | RepoRadar (window 15) | agentic Opus 4.8 baseline |
|---|---|---|
| mean net@2, **25 repositories** | **+5.72** | +1.56 |
| shown / actionable | 212 / 189 | 51 / 47 |
| precision | 0.892 | **0.922** |
| net-negative repositories | 0 | 1 |

**Paired +4.16, 95% CI [+2.44, +6.00], 18 w / 2 l / 5 t, sign *p* = 0.0004.** It is also the first headline covering all 25 repositories: the +5.42/+5.12 split existed only because one baseline run had failed, and its cache entry — stale precisely because failures are never cached — re-ran and succeeded here.
