...
We describe RepoRadar, a system that recommends arXiv papers a repository's maintainers should *act on* — a utility judgment, not a topical-relevance one — and the six-week campaign that measured it. Configured as this paper describes, it reaches mean **net@2 +5.72** against a strong agentic baseline's **+1.84** across 25 repositories (paired **+3.88**, 95% CI [+2.24, +5.60], sign *p* = 0.0007) at one-eightieth the cost per repository. That baseline figure corrects the **+1.56** we first published: our harness scored three of the comparator's repositories as abstentions while a second reader of the same cache file counted their picks, so we were understating the system we compare against **[C-25]**. The margin is sensitive to which model the baseline runs, and we report that sensitivity rather than a single number: against **Claude Opus 5** under a better prompt and a longer budget the same system is paired **+1.52**, 95% CI [−1.04, +4.16], 12 w / 13 l — not significant (§8.4.1). The comparator's strengthening decomposes as +0.32 from the harness and +2.04 from the model, so the published figure is a fair instantiation of the baseline it names; and on the 32 of 37 repositories where the stronger comparator does not over-answer, the two systems are level. What net@2 still credits us with is abstention discipline, which is the metric's own bias stated as a result.

Three findings we believe generalize. **Retrieval for actionability fails on a register mismatch**: a codebase's vocabulary describes what it *has*, while the paper that would improve it describes what it should *adopt*. Every query channel derived from repository text reached 0–8%, while the two that route around the mismatch — a citation hop through the repository's own bibliography, and HyDE-style hypothetical abstracts against a dense index — reach 75% in union (95% CI [61%, 85%]). These are *competitive coverage* rather than recall, and we measure why the distinction matters: the reference set is one draw of one searcher, a redraw reproduces 39% of it, and the population it samples is at least an order of magnitude larger (§5.5). **LLM judges of utility are near-binary in practice**: a 0–3 gate put almost every admitted paper on one score, hiding a per-repository precision spread from 0.00 to 1.00. The literature's remedies — listwise selection, pairwise tournaments, ensembles — all failed here with a consistent excess-strictness pathology, while re-asking the same question on a 0–9 scale and reading the *expectation over the answer token's distribution* ordered the hidden band at ROC-AUC 0.84. **Most of what we learned came from instruments, not ideas**: of roughly sixty measured claims, thirty-eight are negative results and twenty are corrections of our own published numbers.
...
```
profile(R) ──► queries ──► collect (keyword channels ~227 papers,
        range 56–296; roughly 2–3× that with the dense channel on)
...
| +3.88 / +4.12 / +4.12 / +4.16 | 25 | 10 | 50 | live | 08-11…13 | four further draws of the shipped system (the +4.16 draw ran under §9.3's changed query default) |
...
| +5.12 | 25 | **15** | 50 | frozen | 08-15 | the same arm on all 25 — superseded by the row below |
| **−8.12** | 25 | **15** | none | live | 08-16 | **what `rr init` ships** (§8.9) |
| **+5.72** | **25** | **15** | 50 | frozen | 08-16 | **the headline**, at `w_embedding: 1.5` — and the first row with a baseline on all 25 (that baseline column is corrected +1.56 → +1.84, this row unaffected) [C-25] |
...
Repositories never cite what would improve them (NR-1) — but those papers are **one hop away**. Seeding on the arXiv IDs the repository itself cites and expanding one hop in both citation directions reached **18/24 (75%)** of the original gold set, against 0–3 for everything else. Honesty about that number took two more measurements. First, a transport bug had earlier produced 14/24 with a confident wrong mechanism attached (§11). Second, re-measured over the expanded 22-case benchmark, 75% was revealed as recall over a favourable subset: across all cases the hop reaches **21/48 = 44%** (95% CI [31%, 58%]; §5.6 explains why these fractions are estimates rather than counts), with a clean structural predictor — repositories with ≥7 arXiv-cited seeds give 89% recall, thin bibliographies 33%, and six repositories with no arXiv bibliography 0% by construction. Three attempts to extend it failed: coupling-degree pool filtering did not replicate off the concentrated cohort (70% cut → 10%) **[NR-8]**; synthetic seeds from LLM phrases recovered 4/27 with all successes in the single arXiv-native domain (vector search) **[NR-6]**; gap-phrase re-ranking lost to pasting the keyword profile **[NR-7]**.

### 5.4 What works, part 2: HyDE against a public dense index

The remaining channel inverts the register problem: have an LLM write the *abstract of the paper it wishes existed* for this repository (HyDE [4]), and search a dense index with it. A public 3.1M-vector binary-embedding snapshot of arXiv abstracts [6] makes this free beyond one-time 432 MB of column-pruned Parquet range reads. The experiment was gated on verifying four dependencies (license, columnar fetch, latency, target coverage — all passed) plus a fifth we almost failed to name: that vectors we compute are **bit-identical** to the index's (they are, Hamming 0/1024 on held-out rows) — without which every query would have measured nothing while looking healthy. Blind (the generator saw only the repo context), four hypothesis abstracts per repository: union top-1k reaches **27/48 = 56%** [42%, 69%], median rank 837, versus 10/48 = 21% [12%, 34%] for embedding the README (bimodal: occasionally rank-1, median 46,656 — the register mismatch visible as a distribution) and 3/48 for keyword queries. Crucially the two working channels barely overlap: **hop ∪ HyDE = 36/48 = 75%** [61%, 85%], with 15 targets only HyDE reaches — including repositories whose bibliography is empty. Of the two, only the dense channel is shipped and measured end to end (§8.5); the hop's remaining 9 complementary targets fall almost entirely in repositories that already saturate the digest, so its marginal value is real but currently unmeasurable on this benchmark.

### 5.5 What the denominator is, and what these fractions can mean

Every recall figure above divides by a **gold set derived from one run of the agentic
baseline**, and that construction bounds what the numbers are allowed to say. Two
measurements settle it.

**The set is a sample, not an enumeration.** The judge scores ~2% of random arXiv papers
actionable (§3.4), so the actionable population is order 10³–10⁴ per repository; a gold set
of two to five papers per case cannot be a census of it. Redrawing the baseline three more
times over the 25-repository benchmark makes the sampling explicit: a fresh draw of the
*identical* configuration reproduces only **39%** of the frozen targets, and the union of
targets over eleven repositories grows 33 → 50 → 63 → 81 across four draws with no sign of
flattening (Chao1 lower bound ≥ 262). Neither the membership nor the size of the set is a
property of the repositories; both are properties of the draw.

**The estimates, however, are stable.** Four independent gold sets — the frozen one and
three redraws — give pool-reach estimates of 0.14, 0.15, 0.21 and 0.26, mutually overlapping
(pooled 0.19, 95% CI [0.14, 0.25]). Membership churn is *variance, not bias*: a reach
fraction measured against any exchangeable draw estimates the same population quantity. So
the fractions in §5.3–5.4 are legitimate **estimates with sampling error**, and we quote them
with binomial intervals of roughly ±12 points at these denominators. What they do not support
is the comparison of adjacent figures — a move from 36/48 to 43/56 is well inside one
interval and resolves nothing.

**What the metric is, stated plainly.** Coverage of this set is not recall over the
actionable literature, which is unmeasurable and is not what a fifteen-paper digest is
optimising. It is *competitive coverage*: the probability that a paper a rival searcher
demonstrably found, and a judge certified, is reachable by our own channels. Low coverage is
damning — 0/24 was, and correctly drove §5.1–5.2. High coverage is weaker evidence than it
looks, because it partly measures agreement with one searcher's distribution. The
complementary instruments are pool labelling (§5.6) and end-to-end net@2 (§8), and no one of
the three is sufficient alone.

One consequence we report rather than resolve: ~19% of baseline runs exhaust their turn
budget and produce nothing, and two repositories failed in all three redraws while appearing
in the frozen set through earlier successful draws. Every cached answer is conditioned on a
run that *completed*, so repositories whose agent habitually runs long are represented only
by their shorter draws. The direction of that bias is unknown; its existence is not.

### 5.6 The pool is dense, and the bottleneck moves
...
The map is calibrated *to a prompt*, so the repository-description block is one shared function used by the product, the gate, and the benchmark harness; a test asserts the shipped prompt is **byte-identical** to the one the coefficients were fitted against. Failure is designed to be loud and conservative: a paper whose call fails is omitted, never scored (an unscored band paper does not reach Top Picks); a paper arriving with **no abstract** is skipped before the call rather than scored on its title alone, because the prompt describes a candidate by title and abstract and an empty one makes the model answer about a title — an unmeasured paper, not an unactionable one; a run that scores under half of what it *attempted* skips the gate with a warning rather than demoting everything into a fake abstention, the denominator being attempts rather than the band so that deliberate skips cannot make a healthy run read as an outage; scores are persisted only when the gate applied, so post-hoc consumers (archive, notifications) infer gating from data instead of trusting a flag — the flag version, our first attempt, did not even cross the two CLI commands involved, which a linter caught. A parametrized test renders every output format and asserts each honours the threshold; it immediately caught two generators the plumbing had missed.
...
**How much of a headline is weather, measured.** A third draw of the identical configuration (2026-08-11) puts the benchmark's own noise at a residual per-case sd of **1.23** (42 df) plus a whole-run shift of sd 0.27, giving a **minimum resolvable effect of 1.04 net@2 per case** for a paired same-session comparison and 1.07 against a stored run. Two consequences. First, the 22-case mean across three draws is **+4.18** with a range of +3.91 to +4.55, so the +4.55 we published was the highest of three. Second, and worse for single-run reporting: the 25-case sign test gives *p* = 0.0414 on one draw and *p* = 0.0001 on the next, **two orders of magnitude apart on identical inputs**. A single run's significance is not a property of the system. (Both draws are among the four that C-25 restates; corrected they read *p* = 0.1153 and *p* = 0.0007, which sharpens rather than softens the point — the first draw does not clear 0.05 at all, and the spread is unchanged at roughly two orders of magnitude.) We also note what this floor implies about our own null results: the stated-intent experiments (+0.44, +0.12) were below the resolvable effect before they were run, so their nulls are real but uninformative. **[C-8]**

**Every number in this section describes a ten-paper digest, and the system ships fifteen.** The benchmark's returned-set cut and the product's `output.top_n` had drifted apart the same way the gate depth had (§8.9), and in the same audit. Measured on one frozen pool with the width as the only variable, widening it is worth **+1.24 net@2 per case** — so the figures above understate what a user receives, and the paired advantage over the agentic baseline grows with them. Re-measured against the cached Opus responses at the shipped width, RepoRadar reaches **+5.72 against +1.56, paired +4.16** (95% CI [+2.44, +6.00], 18 w / 2 l / 5 t, sign *p* = 0.0004) across all 25 repositories — the baseline column of which is understated, and is corrected to **+1.84 / paired +3.88** below **[C-25]**. That figure carries `ranking.w_embedding: 1.5`, worth +1.00/case over the 0.0 every earlier headline ran at (§8.10); the 24-repository predecessor read +5.42 against +1.62 because one baseline run had failed, and that run has since completed.

We report that with the precision line first, because it is the more informative one: at fifteen the system returns **four times as many papers at nearly six points lower precision** than the agent (0.888 against 0.945 — the agent's column reads 0.938 as that run measured it, and 0.945 once C-25's forfeited picks are restored, so the correction widens this gap rather than the margin above). net@2 rewards each actionable paper linearly and charges 2 for a dud, so a wider digest at slightly worse precision wins by a wide margin — and the metric's volume sensitivity, already flagged in §4.1 and §8.5, is larger at fifteen than at ten. What licenses showing those papers is that their marginal precision (0.855) clears the 2/3 breakeven the metric derives; what it does not license is the claim that the digest is *better to read*. Those are different propositions and only the first is measured here.
...
*(Correction, 2026-08-23: the measured column is the arm this comparison actually ran beside — `w_embedding: 0.0`, the value every published run to that date had set. At the §8.10 weight the same preset reads +5.72 with 212 shown / 189 actionable, precision 0.892, 0 net-negative. This table previously printed the +5.72 mean above the +5.12 run's composition rows — the C-17 shape again, inside the section that documents it.)*
...
### 8.10 The third shipped default, and the only one where the benchmark was the weaker side

§8.9 closed with three unmeasured shipped values. Two were reported there. The third, `ranking.w_embedding`, is the only field in the audit whose two surfaces disagree on a *number* rather than a boolean: `1.5` in the configuration `rr init` writes, `0.0` in the dataclass and in every published result to that point. We predicted a loss — the sole prior measurement of that channel scored README embeddings as a retrieval *query* and found them bimodal, median rank 46,656 — and pre-registered −1.5 to +0.5, most likely negative.

Two paired draws over one frozen pool gave **+1.00 net@2 per case** (CI [+0.14, +2.08], sign *p* = 0.035, past the two-draw floor of 0.52). The prediction failed, and the reasoning error is worth more than the result: **median rank is the wrong statistic for a component that only breaks ties near the top.** As a query, a bad embedding match costs the entire result set; as one weighted component over an already-retrieved pool, a mediocre signal is diluted and only its rank-1 tail is visible. The statistic did not transfer between the two roles, and we moved it without checking that it did.

**What the result inverted.** We had opened the question expecting to find the product shipping a degradation to its users. The opposite held: the template's `1.5` was the better value, and the configuration this paper *recommends* carried the worse one — so every headline up to that point was understated by about a point for any user with the optional `embeddings` extra, which the recommended setup installs.

**Why a resolved delta was not enough to move the number.** The obvious next step — write `1.5` into the record of what the benchmark measures — is the C-17 error. That record states what published runs actually *set*, and they set `0.0`; editing it on the strength of a delta asserts a run that never happened, inside the artifact whose purpose is preventing exactly that. Our own pre-registration had said to do it, having conflated "the configuration we endorse" with "the configuration we measured". So the headline was re-measured instead.

**The re-measurement, and a free prediction that made it cheap.** Both draws had run without the agentic baseline, so neither carried a paired comparison. Supplying one required no new baseline run, because net@2 is a function of a system's own returned papers — pool composition drives nDCG and the abstention rule, not the score. Predicting the paid run from that invariant, then paying for it, cost \$4.38 and tested the invariant at the same time: the baseline column reproduced to the digit (+1.62/case, 48 shown, 45 actionable) across two different candidate pools, and RepoRadar's own side landed +5.92 against a predicted +6.23, inside the one-draw floor of 0.74.

| | RepoRadar (window 15) | baseline, as measured | baseline, corrected **[C-25]** |
|---|---|---|---|
| mean net@2, **25 repositories** | **+5.72** | +1.56 | **+1.84** |
| shown / actionable | 212 / 189 | 51 / 47 | 58 / 54 |
| precision | 0.892 | 0.922 | **0.931** |
| net-negative repositories | 0 | 1 | 1 |

**As measured: paired +4.16, 95% CI [+2.44, +6.00], 18 w / 2 l / 5 t, sign *p* = 0.0004. Corrected: paired +3.88, 95% CI [+2.24, +5.60], 17 w / 2 l / 6 t, sign *p* = 0.0007.** It is also the first headline covering all 25 repositories: the +5.42/+5.12 split existed only because one baseline run had failed, and its cache entry — stale precisely because failures are never cached — re-ran and succeeded here.

**Why there are two baseline columns, and why we did not simply replace the first.** Three repositories — `compiler`, `graph`, `storage` — score `n_returned = 0` in that run, and in every run since. Their cached baseline responses hold a **128-character restoration note** where the transcript used to be: a 30-turn re-run displaced the original 12-turn entry, and only the identifiers could be recovered, from a recorded run file that keeps ids and verdicts but not the model's answer. The runner re-parses the cached answer on every cache hit — deliberately, so that a parser fix reaches already-cached runs — and re-parsing a note yields nothing. Meanwhile the pool diagnostic reads the `ids` field of the *same file* and counts seven papers there, all judged actionable, all of them gold targets for the recall figures of §5. **One cache, two consumers, opposite answers, for six weeks.** The comparator was forfeiting seven of its own correct picks, worth +0.28 net@2 per repository, and every paired margin we published was that much too generous.

**The mechanism that destroyed those three transcripts is now closed, and it took a second prompt to notice it.** The cache discriminator hashes the model, the prompt and the flags, so a configuration change correctly marks a stored entry stale — but the *path* it is stored under carried none of that, and a stale entry is re-run and written back to the same file. Invalidation and displacement were the same operation. Nothing in the design said so, which is why the 30-turn re-run of 2026-08-09 looked like a re-measurement rather than a deletion. Extending the baseline to a second prompt would have repeated it across all 34 caches at once; the prompt version is therefore part of the cache location and not only of the hash, so the two configurations sit beside each other and a future one cannot overwrite either.

The fix for the reading half is a fallback keyed on the *absence of a recommendation block* rather than on the absence of parsed ids, and the distinction is the whole design. `webdev` writes "My recommendation is to recommend nothing", emits an explicit `[]`, and still carries four identifiers an older parser scraped out of its prose — including a bare URL path that is not an arXiv id at all. An empty array is an **answer**; a restoration note is not a reply. A fallback keyed on "no ids parsed" would have resurrected exactly the picks the authoritative-block rule exists to discard, and it was our first attempt. Both halves are now pinned against the real artifacts.

We keep both columns because +1.56 is what the cited run file contains, and every earlier document of ours quotes it. Overwriting a measured value with a better one — even a correctly better one — is the C-17 error in the section that documents C-17. What moved is the accounting, not the run: net@2 reads a system's own returned papers, so RepoRadar's +5.72 is untouched and the entire correction is a transfer from our margin to the baseline's column. Significance survives here (*p* = 0.0007, CI floor +2.24) and the qualitative claim is unchanged, but the margin is 7% smaller than we said.

#### 8.4.1 The margin against three comparators **[P26]**

The correction above changes the comparator's *accounting*. This subsection changes the comparator itself, which turns out to matter roughly seven times more. After the measurement window closed we ran two further baselines over the same 25 repositories: the same model under a better harness, and a newer model under that harness. Each system is scored on its own returned papers, so RepoRadar's +5.72 is again untouched.

| comparator | net@2 | papers/case | precision | abstains | paired vs RepoRadar |
|---|---|---|---|---|---|
| Opus 4.8, v1 prompt, 12 turns — **the published comparator** | +1.84 | 2.2 | 0.931 | 6 | **+3.88**, CI [+2.24, +5.60], 17 w / 2 l / 6 t, *p* = 0.0007 |
| Opus 4.8, v2 prompt, 30 turns | +2.16 | 4.2 | 0.838 | 3 | +3.56, CI [+1.64, +5.52], 15 w / 4 l / 6 t, *p* = 0.019 |
| **Claude Opus 5**, v2 prompt, 30 turns | **+4.20** | 9.1 | 0.820 | 0 | **+1.52**, CI [−1.04, +4.16], 12 w / **13 l** / 0 t, *p* = 1.00 |

**The margin survives a harness upgrade and does not survive a model upgrade.** Both Opus 4.8 rows clear *p* < 0.05; the Opus 5 row does not, on any RepoRadar arm we can put against it. Its sign test is exactly 1.00 because the win count is 12 to 13 — the mean is ahead and the case count is not, which is the shape a few large wins produce and which a mean alone conceals.

**The published comparator was not under-resourced.** This is the obvious objection to a baseline that recommends 2.2 papers where ours shows 8.5, and it is answerable by decomposition rather than assertion: holding the model fixed and granting the v2 prompt and the 30-turn budget is worth **+0.32** to the comparator; exchanging Opus 4.8 for Opus 5 is worth **+2.04**. Six-sevenths of the strengthening is the model. The +1.84 we report is a fair instantiation of the system it names, and what has changed since is not our generosity but the frontier.

**What the remaining margin is made of.** Extending the Opus 5 comparison to all 37 repositories (the 25 here plus the twelve scientific ones of §10.3) gives +1.08, CI [−0.97, +3.22], 20 w / 15 l. Split by whether the comparator over-answered: on the **32 repositories where it does not, the two systems are level (−0.06)**; the entire margin comes from **five** where it does, four of which are repositories RepoRadar declines to answer at all while Opus 5 returns 5–20 papers and averages −7. On the shipped arXiv-only configuration over all 37 the record is **18 w / 18 l / 1 t**. So what net@2 is measuring at this point is abstention discipline, which §3.2 names as the metric's own bias and §8.4 shows is worth thirteen points against a display rule that cannot abstain. It is a real property of the system and it is not the claim that the system finds better papers.

We report the full three-by-three table — three comparators against the published arm, a fresh same-day arXiv-only control, and an arXiv+EPMC arm — in `evals/comparator_ladder.json`, derived by `evals/restate_comparator.py` from the same two statistical helpers every figure in this paper uses. The unshipped arXiv+EPMC arm scores +0.32 higher against every comparator and flips no significance verdict.

**The blast radius is every paired run after 2026-08-09, and one of them does not survive.** The caches were damaged on a fixed date, so the question is answerable exactly rather than by judgment: four runs read them, and `evals/restate_c25.py` restates all four.

| draw | paired, as measured | corrected | sign *p*, as measured | corrected |
|---|---|---|---|---|
...
The comparator gains +0.28 to +0.30 per repository in every one, which is the reassuring half: the defect is a constant, not a draw-dependent one. The unreassuring half is the first row. **A draw we published as significant at *p* = 0.0414 is not significant once the comparator is scored correctly** — and it is the draw §8.7 already singles out as the weak one, so the correction sharpens a caveat we had made rather than introducing a new one. It does mean that of four independent draws of this system, one fails to separate from the baseline at all. That belongs beside the headline, not in a footnote.

Two smaller consequences. The baseline's precision rises with its picks — 0.938 → 0.945 on the 08-14 draw — so §8.7's "four times as many papers at lower precision" caveat gets *worse*, from five points to nearly six. And the run artifacts, we discovered while doing this, **never recorded which baseline mode produced them**; the four runs above are identified as `cli` by matching their picks back against the caches (45/45, 48/48, 48/48, 51/51). Given that we also measured `cli` and `api` as different systems entirely — 64 picks against 34, 10 shared — a results file that cannot say which comparator it ran against is a gap of the same family as the one this correction is about.

**A claim made from the free prediction and then withdrawn.** The two draws showed precision 0.899 and 0.913 against the published 0.888, and we reported that the weight narrows this system's largest caveat: it returns four times as many papers as the baseline at lower precision. The paid draw reads 0.891. The volume increase replicates (208, 208, 211 against 195, 193, 196); the precision gain does not, and two draws should not have been read as a pattern. The caveat stands unchanged — four times the papers at three points lower precision, and whether a maintainer prefers that is a question net@2 does not answer. Likewise "0 net-negative repositories" is this draw's value and not the method's, the distinction C-7 was written to enforce.

**What did not move.** The dataclass default stays at `0.0`, and the surface split is now deliberate rather than unexamined: that value is what the keyless out-of-the-box arm was measured at (−8.12), and raising it would place an unmeasured value underneath a published negative number. The field also remains the only one whose behaviour depends on the install, which is a promise a default cannot make.
...
> Communications*, *npj Computational Materials*, *The Journal of Chemical Physics*, *Physical Review B*. The OpenAlex figure quoted
...
- **NR-38** `ranking.w_embedding`, the last value shipping to users that no number covered: **1.5** in the config `rr init` writes, **0.0** in the dataclass and in every published result. Measured on one frozen pool it is worth **+0.64 net@2/case**, CI [−0.28, +1.80], sign p = 0.79 — inside the 0.74 floor on that draw. A **second paired draw** over the same pool gave **+1.36** (CI [+0.40, +2.52]); combined under a decision rule fixed beforehand, **+1.00 net@2/case, CI [+0.14, +2.08], sign p = 0.035, past the two-draw floor of 0.52 — resolved and positive**. Volume improved (208 papers shown against 195, 187 actionable against 173); precision looked improved too (0.899 against 0.887) and did not replicate — a third draw at 0.891 withdrew that half of the claim (§8.10). Our pre-registered prediction was −1.5 to +0.5 and **most likely negative**, reasoning from the one prior measurement of this channel — README embeddings as a *query* are bimodal, median rank 46,656. The transfer was invalid: as a query a bad match costs the whole result set, while as one weighted component over an already-retrieved pool a mediocre signal is diluted and its rank-1 tail is what shows. **Median rank is the wrong statistic for a component that only breaks ties near the top.** The result inverts the concern that motivated the experiment: the default template's 1.5 is the better value and the configuration we *recommend* carries the worse one, so every headline here is understated by about a point for users with the `embeddings` extra installed — which the recommended `.[hyde]` setup pulls in. Nothing was edited in response: the record of what the benchmark measures states what published runs actually set, and rewriting it on the strength of a delta would assert a run that never happened. The headline was re-measured at 1.5 instead, which closed it (§8.10).
...
**Corrections of our own published numbers** — each recorded in place, with the wrong number retained as a warning. The identifiers are the project ledger's, and the ledger runs further than this list: the gaps (C-20 through C-24) correct probes and findings that never reached a figure in *this* paper — a probe's own statistics, a negative result we did not carry over, a coverage denominator. They are recorded in the results log rather than omitted.
...
- **[C-25]** **The headline understated the system it compares against.** Three baseline caches hold a 128-character restoration note where their transcript used to be; the runner re-parses the cached answer on every hit, a note parses to nothing, so those repositories scored as abstentions — while the pool diagnostic read the `ids` field of the same files and counted seven picks there, every one judged actionable and every one a gold target for §5's recall figures. One cache, two consumers, opposite answers, unnoticed for six weeks because *both* readings looked reasonable in isolation. The comparator rises +1.56 → **+1.84** and our paired margin falls +4.16 → **+3.88** (*p* = 0.0004 → 0.0007); the headline claim survives, 7% smaller. All four paired runs after the damage date are affected, and the 2026-08-10 draw **loses significance entirely** (*p* = 0.0414 → 0.1153), so one of four independent draws of this system no longer separates from the baseline. The near-miss is the instructive half: the obvious fallback — "use the stored ids whenever the replay yields none" — would have resurrected four identifiers an older parser had scraped from the prose of a repository that explicitly recommended nothing, one of them not an arXiv id at all. **An empty array is an answer; a restoration note is not a reply**, and only the second deserves a fallback (§8.10).
...
14. **A failure that does not raise where it happens.** `subprocess.run(text=True)` decodes a child's output with the *locale* codec, and when a byte will not decode, CPython's reader thread raises — then `subprocess.run` prints that traceback and swallows it, handing back a completed process whose `stdout` is `None`. The program dies four frames later inside `json.loads`, with a message about JSON, having already paid for the model answer it threw away. The general shape is worth naming: a library that converts an exception into a sentinel moves the evidence away from the cause, and no amount of care at the crash site recovers it. The repair is a property rather than a patch — *no captured output may be decoded with an unpinned `errors` handler* — and, as in [C-9b], the survey mattered more than the fix: seven further call sites had it, one of them five `git` calls sitting beside a sixth that had already been repaired in isolation, in the module that produces our only model-free ground truth. The handler cannot be uniform either, which is the part a blanket fix gets wrong: where output is read, edited and written **back** — our scheduler round-trips the user's whole crontab — `errors="replace"` silently overwrites other people's entries with U+FFFD, and is a worse bug than the crash. **[C-26]**
15. **A denominator you built yourself is a sample, and nobody ever checks whether it reproduces.** Our gold set is the standard pooled-relevance construction — run a system, judge what it returns, divide by that — and we quoted fractions against it for six weeks without asking the one question the construction invites: *run it again, do you get the same set?* You do not. A redraw of the identical configuration reproduces **39%** of the targets, the union across draws does not saturate, and the population being sampled is at least an order of magnitude larger than the sample. The reflex on discovering that is to make the set bigger — more draws, union them — and it is wrong twice over: a union converges to *everything one searcher finds*, importing that searcher's biases wholesale, and it mechanically depresses any coverage fraction whose numerator is capped by a digest width. The repair is to stop treating it as a set. **Coverage of a pooled reference is a probability estimate, not a count ratio**: report it with a binomial interval, let extra draws tighten the interval rather than move the denominator, and check exchangeability by confirming that independent draws give overlapping estimates (ours do — 0.14, 0.15, 0.21, 0.26 — which is what licenses the fractions at all). The generalisable warning is that the arithmetic looks identical either way: `43/56` is a defensible estimate and an indefensible count, and nothing in the notation tells you which one you have written. **[P17]**
...
**The reference set is a sample of one searcher's reach.** Gold targets are papers the
agentic baseline found and the judge certified, so "recall" against them is competitive
coverage, not coverage of the actionable literature (§5.5). Three redraws put numbers on the
consequences: membership is 39% reproducible, the union does not saturate (Chao1 ≥ 262 over
eleven repositories holding 33 frozen targets), and ~19% of baseline runs exhaust their turn
...
in exactly one of them, and Chao1 puts the population that *one configuration* can certify at
**≥ 237**. Three draws of our strongest searcher found under 40% of what it alone can find,
