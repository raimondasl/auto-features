...
* **P5 was scoring `met: True` and the registered prediction had failed.** The test was `capped_positives >= 100 and walked <= summary["budget"]` — the budget the *run* used, read out of `walk_summary.json`. §3.4 lets that budget grow, and it did: raised to 6,000 for the §3.3 extension, `3689 <= 6000` was true and P5 passed on 157 positives that took **3,689 rows** to reach. Within the registered B = 1,200 the walk had **60**. This is the hazard `walk_stop_reason` was already hardened against in this same file — "reading the thresholds out of it let the gate take its bar from the very artefact it is gating" — and the scorer was left reading the same unguarded field, so an operational decision about budget silently moved a registered bar. It now scores against `walk_pool.DEFAULT_B`, from the yield curve rather than the ledger (whose `capped` column is a mining-time snapshot the contest can later un-count). **P5 is not met.**
* **P6 was emitting no verdict at all.** Above its registered n it computed `auc`, `both_exclude_half` and `difference_excludes_zero` and stopped: the brackets lived in the prediction sentence and were never compared to the AUCs, and `difference_excludes_zero: True` — which is the prediction failing — was reported with nothing saying so. Every clause is now scored and `met` rendered, with an unscorable clause leaving the whole prediction unscored rather than dropping out of a conjunction. **P6 is not met**: 0.9215 and 0.9424 against brackets of 0.60–0.70 and 0.62–0.72, and the difference does exclude 0.
