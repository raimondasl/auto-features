...
| does an AGENT with RepoRadar attached beat either alone? (P27) | `mcp_arm_report.py` ($0; derives evals/mcp_arm.json from the frozen arXiv+EPMC run, gold_spread_v2_opus5.json, and -- once run -- gold_spread_v2_opus5_web_rr.json. Pinned by tests/test_mcp_arm_report.py; pre-registered in evals/PREREG-mcp-arm.md. Four columns: A RepoRadar published, A' RepoRadar as the product DISPLAYS it (minus 11 of 325 picks the repository already cites, worth +0.05/case CI [-0.14,+0.24] -- estimated at +0.22 first by counting only the actionable side, and wrong by a factor of four), B Opus 5 alone, C Opus 5 with the MCP server. Reproduces P26's A-B as +1.08 CI [-0.97,+3.22] from an independent code path. **C is recorded `not_run`, never 0** -- an unrun arm scored as zero reads as 'the agent recommended nothing') |
| does the judge track ADOPTION on a population enumerated for the purpose? the pre-registered validity pool (PREREG-judge-validity-pool.md §5) | `judge_validity_pool.py` ($0 for the boundary; the endpoints cost verdicts once the walk has run. **The §5 analysis wrapper, named at registration as the one honest gap: it must exist BEFORE any verdict is bought.** What is committed so far is the artefact boundary — `FROZEN_RECORDS`, `artifact_path(source, scheme)` and `write_artifact`, which refuse to overwrite the published NR-56/57 record, refuse an unmarked file, refuse a non-finite number, and hash the published records on both sides of every write. `report()` in `judge_validity_adoption.py` now writes through it, so the NR-57 reproduction lands in .work/repro/ instead of replacing what it reproduces. Pinned by tests/test_judge_validity_pool_artefact.py. The primary endpoint (repository-clustered AUC), the secondary gap, transportability and the contamination split are NOT yet written) |
| is the judge VALID, not just reliable? adoption ground truth (NR-56, NR-57) | `judge_validity_adoption.py` (~$5, 310 verdicts; --plan, --judge, then bare. Scores the 31 git-mined adoptions AND 124 matched controls (same repo, published before the same T0, never adopted) against the T0 context with both judges; the statistic is the GAP, because recall alone rewards a lenient judge. Wrote evals/judge_validity_adoption.json, pinned by tests/test_judge_validity_adoption.py -- that file is now a FROZEN published record and this script no longer writes it: the re-run lands in .work/repro/ via judge_validity_pool.artifact_path, so a reproduction can disagree with what it reproduces. gpt-5.5 gap 0.153 CI [-0.040,+0.339] SPANS ZERO and calls 49.2% of controls actionable; claude-sonnet-5 gap 0.282 excludes zero; the difference 0.129 misses the registered 0.15 bar so no better instrument is named. NR-57 re-ran it on 35 adoptions / 140 controls after mining all 37 cases, with controls re-seeded PER CASE so the sample is stable under growth: gaps 0.143 and 0.243, same three conclusions, so NR-56 was not an artifact of one control draw. 55 adoptions would settle it and the channel tops out at 35) |
| did temperature=0 make the gate deterministic? (NR-55) | `temperature_zero_check.py` ($0 to re-derive; needs two post-fix runs against the same frozen pool. Writes evals/temperature_zero_check.json, pinned by tests/test_temperature_zero_check.py. 31/37 byte-identical against 10/37 pre-fix, per-case sd 0.37 against 1.44 -- 3.9x tighter; 33/37 deterministic in everything net@2 reads. The 37/37 prediction was WRONG and is recorded as such. Half-width 0.78 -> 0.64 against NR-54's projected ideal of 0.63) |
| how much run-to-run variation is the GATE, not the pool? (NR-54) | `gate_draw_variance.py` ($0 to re-derive; needs one replicate run against the SAME frozen pool. Writes evals/gate_draw_variance.json, pinned by tests/test_gate_draw_variance.py. Corrects the 2.23 sd quoted after NR-52 -- that was measured across DIFFERENT pools (median Jaccard 0.365). With the pool held byte-identical the gate/downstream sd is 1.44 against an implied pool component of 1.71: GREY on the pre-registered reading. The gate is 35% of paired variance, so temperature 0 would tighten +-0.78 to +-0.63, a fifth, not enough to make ladder rungs individually resolvable) |
| how much of the cross-judge gap is Sonnet's own sampling noise? (NR-53) | `sonnet_self_agreement.py` (~$3; `--plan`, `--judge`, then bare. Re-draws 200 random NR-52 papers from the SAME judge at the same settings, cached separately via second_verdict(cache_as=...). Writes evals/sonnet_self_agreement.json, pinned by tests/test_sonnet_self_agreement.py. MUST run before the temperature is fixed -- at temperature 0 self-agreement is trivially ~1.0. Result: self-kappa 0.798 against a cross-judge 0.199, label flips 8.4% -- PASS, so NR-52's judge-dependence is real and not a sampling artifact) |
| did the identifier line move NR-52's Sonnet verdicts on the baseline's arXiv picks? (PREREG-sonnet-id-probe.md, NR-67: Immaterial) | `sonnet_id_probe.py` (~$8 of Sonnet, 474 verdicts, capped at 600 calls; `--plan` (no judge call, free arXiv resolution) then `--judge` then `--report`. Pre-registered in evals/PREREG-sonnet-id-probe.md before any verdict existed. Re-judges Opus 5's 237 arXiv picks twice with claude-sonnet-5: arm V shows the versioned id GPT-5.5 saw, arm U the unversioned pick id NR-52 sent. Title, abstract and context are shared byte for byte, and `--plan` checks that the identifier line is the only prompt difference. Verdicts cache under .work/second_judge/claude-sonnet-5#id-versioned and #id-unversioned; resolved texts and the call ledger under .work/sonnet_id_probe/. Writes evals/sonnet_id_probe.json (E1 V-minus-U actionable rate, E2 and E4 margin shifts with a case bootstrap, E3 replicate flip rate against NR-52, voids by arm, the registered reading), pinned by tests/test_sonnet_id_probe.py, whose artifact tests skip until that file exists) |
| where does a third vendor's judge fall between GPT-5.5 and Sonnet? (PREREG-third-judge.md, NR-69: below both on level, orders the band) | `third_judge.py` ($6.45 of gemini-3.8-flash, 1,536 calls, where ~$11-28 was estimated over 1,534 distinct prompts, capped at $40 and 3,000 calls; `--plan` (no paid call: free DOI resolution and free countTokens) then `--judge` then `--report`. Pre-registered in evals/PREREG-third-judge.md before any verdict existed. Sends judge.RUBRIC plus judge._build_user_prompt as one user message through `gemini_client.py`, a standard-library REST client inside evals/ used by nothing else, at thinkingLevel MEDIUM, maxOutputTokens 16384 and the four adjustable safety categories at BLOCK_NONE; the prompt is built by the same two functions the Sonnet path calls, and `--plan` rebuilds the Sonnet side per sampled paper and diffs it. That diff shows the two constructions agree: outside the baseline both sides start from the same context and paper object, so it is not evidence that the stored Sonnet verdict saw those bytes, and for the baseline's 120 DOI picks no Sonnet-side text was ever stored to compare. `--plan` prints that count per population and, per population, how many prompts carry a paper id that the second judge's cache has a verdict filed under. Four populations, an identical prompt asked once: the 324-paper 2026-08-20 score-2 band, NR-52's two arms (ours 306 and the baseline's 357, its arXiv picks under the versioned ids of NR-67 and its DOI picks resolved once through verify.resolve_references), NR-61's 188 adoptions and NR-62's 502 distinct cross-repository pairs at their T0 contexts. Verdicts cache under .work/third_judge/gemini-3.8-flash/&lt;prompt sha256&gt;.json with the call ledger beside them. The registration, third_judge.py and gemini_client.py are frozen together: `--judge` and `--report` both refuse unless all three are committed and identical to HEAD, and the artifact records the HEAD sha and each file's blob sha. `--report` refuses on everything else `--judge` does too (drifted contexts, a population off its registered count, a paper with no prompt, a case list that moved since `--judge`), skipping only the key check. A 429 is not charged against the 3,000-call cap and twenty in a row stop the run as a tier or quota problem. `--report` first reproduces NR-66's 0.873/0.494 and AUC 0.729/0.702, NR-52's +0.32/-3.41 and NR-61/NR-62's 0.819/0.644 and 0.255/0.068 from the existing labels and refuses if any of them moved, then writes evals/third_judge.json: E1 band level, E2 band ordering, E3 adoption, E4 NR-52's margin under a Gemini-at-2 label, each with a 10,000-draw repository bootstrap, the registered first-match readings, voids by population and cause, and the prediction check, which scores no endpoint whose population is Unreadable. The Unreadable rule is 5% void per population, and on the comparison it is per arm: either arm over 5%, or the two arms more than 3 points apart. Pinned by tests/test_third_judge.py, whose artifact tests skip until that file exists) |
| what do the third judge's verdicts show beyond its registered endpoints? (NR-69, descriptive and post hoc) | `third_judge_followups.py` ($0, offline, tracked inputs only. Joins evals/third_judge.json's Gemini verdicts to NR-66's aug20 band rows in evals/judge_dependence.json. Recomputes the rescore stage's value under all three judges on NR-66's own draws and refuses to write unless the GPT-5.5 and Sonnet figures come back exactly; cross-tabulates the three judges' acceptances with kappa and its ceiling per pair, reproducing NR-68's 0.199 and 0.248; and cuts the comparison under Gemini as NR-68 cut it, with and without the controls and by development and later cases, and each pair of judges' margins against each other on the same runs, on NR-52's paired_bootstrap after reproducing E4 and the published margins. Writes evals/third_judge_followups.json, pinned by tests/test_third_judge_followups.py) |
| is the Opus 5 margin a property of RepoRadar or of GPT-5.5? rung 1 (NR-52) | `rung1_second_judge.py` (~$8 of Sonnet, 460 verdicts; `--plan` then `--judge` then bare for the margins. Pre-registered in evals/PREREG-rung1.md and committed BEFORE any margin was computed. Writes evals/rung1_second_judge.json, pinned by tests/test_rung1_second_judge.py. Three labels: gpt +0.32, consensus +0.57, sonnet-only -3.41. The bar PASSES and is near-unfalsifiable -- consensus demotes 0.7%/1.7% of GPT-actionable papers. Under Sonnet our digest runs 0.585 precision, BELOW the 2/3 break-even, while Opus 5 runs 0.714, above it) |
| how far does the Opus 5 comparison (NR-52) depend on the penalty, the controls and the cases? (descriptive and post hoc) | `comparison_sensitivity.py` ($0, offline, tracked inputs only. Recovers each arm's actionable and unactionable counts per case from evals/rung1_second_judge.json, then sweeps net@lambda over lambda 1, 1.5, 2, 3 and 4 with paired_bootstrap intervals and the exact lambda at which the margin changes sign. Also the margin with and without the three negative controls, on the 22 development cases against the 15 later ones, and the ceiling on the aug20 band's GPT-Sonnet kappa from evals/judge_dependence.json. Writes evals/comparison_sensitivity.json, pinned by tests/test_comparison_sensitivity.py. Under GPT-5.5 the margin changes sign at lambda 10/7, and the controls, where RepoRadar shows nothing, carry +11 of its +12. Without them it is +0.03 [-2.15, +2.29] under GPT-5.5 and -5.35 [-8.41, -2.32] under Sonnet. Under Sonnet the margin is negative at every lambda. Kappa 0.199 sits under a ceiling of 0.248 set by the two judges' marginals) |
| does the Haiku->Luna gate swap survive the second judge? (NR-63) | `gate_swap_second_judge.py` (~$1.30 of Sonnet, 86 fresh verdicts of 401; `--plan` then `--judge` then bare. Writes evals/.work/gate_swap_second_judge.json. Same three labels as rung1_second_judge, imported not reimplemented. All three show no gate difference in the SHIPPED digest -- gpt +0.08, consensus +0.08, sonnet-only +0.16, every CI covering zero. The gate itself differs: Luna admits 12.6/case against 10.9 and the fine-scale rescore removes exactly those, which is why the sweep and the digest disagree in sign) |
| does PRF-HyDE move net@2? item 12's paid arm (NR-51) | `freeze_prf_arm.py` ($0 to re-derive; the arm cost ~$15 — one 37-case run whose only change is a pinned hypothesis file carrying round 1 UNION round 2, 8 abstracts/case through the shipped hyde.discover at top_k 100, against the shipped arm as control. Writes evals/prf_arm.json, pinned by tests/test_prf_arm.py. The pre-registered kill fires at -0.19 against a |0.78| bar, so item 12 CLOSES. The digest churn inverts NR-50's free prior: 85 added at 0.882 displacing 77 at 0.935, dp = -0.053 against a predicted +0.054) |
| do round 2's papers reach the gate? item 12 follow-up (NR-50) | `prf_rank_probe.py` ($0 + ~35 min CPU; `--search` then `--collect` are one-time. Merges round-2 candidates at top_k 100 into the WHOLE shipped pool and re-ranks through the shipped `rank_candidates`, asking what share of the top-50 gate window they take — the constraint NR-47/NR-48 identified. Writes evals/prf_rank_probe.json, pinned by tests/test_prf_rank_probe.py. Pre-registered WITH an effect size: kill <5%, licence >=16%; measured 20.61%, so the paid arm is LICENSED. The share is a magnitude, not a direction — the free quality prior puts dp at +0.054, implying +0.28 net@2/case, inside the noise. Also fills the depth grid `gate_depth` 25/50/100/150/300 from one pass — a cell NR-48 never ran at the shipped top_k — and the share RISES with depth (17.6% -> 26.6%), so round 2 is MARGINAL: 15.2% of ranks 1-10 against 20.6% of ranks 1-50) |
| does a feedback-seeded 2nd HyDE round aim better? item 12 stage 1 (NR-49) | `prf_hyde_reach.py` (~$0.05 one-time via `--hypotheses`, then $0 + ~12 min CPU. Seeds round-2 hypotheses from what the shipped arm SHOWED, then simulates reach BUDGET-MATCHED: round 1 @ 100 against round 1 @ 50 union round 2 @ 50, because NR-47/NR-48 already closed the volume lever. Writes evals/prf_hyde_reach.json, pinned by tests/test_prf_hyde_reach.py. Clears the pre-registered bar by 3 witnesses of 520 at McNemar p = 0.68 — one tenth of NR-46's measured redraw noise — so the pass is REFUSED and no paid arm is licensed. The gains are real but post hoc: 5 sit past rank 1000, beyond any cut NR-47 measured) |
| does a wider HyDE cut move net@2? item 10 stage 2, the paid arm (NR-47) | `freeze_hyde_cut_arm.py` ($0 to re-derive; the arms themselves cost ~$25 — two same-day 37-case runs at hyde.top_k 100 vs 1000 sharing one pinned hypothesis set. Writes evals/hyde_cut_arm.json, pinned by tests/test_hyde_cut_arm.py. The kill condition fires at −0.78, and the diagnostic says the added papers are fine while the digest shrank — so the follow-up is gate_depth, not retrieval) |
| would a wider HyDE union reach more witnesses? item 10 stage 1 (NR-46) | `hyde_cut_reach.py` ($0 + ~$0.20 one-time; `--hypotheses` generates the 17 missing sets, then simulates reach at wider cuts with NO pool re-collection — a witness counts as reached at cut K if already pooled or its HyDE rank is below K. Writes evals/hyde_cut_reach.json, pinned by tests/test_hyde_cut_reach.py. Pre-registered bar ≥ 0.25 at K=1000, measured 0.4481. Reach is a lower bound and says nothing about net@2 — see NR-11) |
...
| how much of the digest is a property of the LABELLING judge? (NR-59) | `judge_refit.py` ($0, no LLM/judge; refits the fine-scale map on Sonnet labels instead of GPT, leave-one-repo-out, at the fixed 2/3 threshold. Reads evals/.work/second_judge_band.json, writes evals/judge_refit.json, pinned by tests/test_judge_refit.py. **The registered test did not run** -- its blocking reproduction check failed 0.799 vs 0.90, because the shipped map was fitted on a wider population than the band it is applied to, so the 92.9% flip rate is recorded and NOT read. What IS established needs no fitting: the judges' AUCs against the fine-scale score differ by 0.027 while their base rates differ by 0.380 -- they order alike and level differently, so every threshold is a bet on a base rate and combining judges picks one rather than measuring it) |
| does the ranker's dense EMBEDDING discriminate actionability, where NR-42's two LLM instruments could not? (NR-58) | `embedding_discriminator.py` ($0, no LLM/judge; reads the judge verdict cache and every frozen pool that carries paper text, writes evals/embedding_discriminator.json, pinned by tests/test_embedding_discriminator.py. **No, and the item stays closed on a second instrument**: AUC 0.578, CI [0.415, 0.673], against a registered bar of 0.65 excluding 0.5. The arXiv control is 0.586, so the embedding is a weak actionability signal everywhere rather than one that fails off arXiv. The finding is the diagnostic: the same signal reads **0.096** on NR-42's shown-only panel and **0.612** on papers the pipeline passed over, intervals disjoint -- an instrument evaluated on the set it helped select looks worse than it is) |
