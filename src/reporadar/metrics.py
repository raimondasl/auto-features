"""Ranking-quality metrics shared by ``rr eval`` and the ``evals/`` benchmark.

All functions take ``ranked_labels`` — a list of binary relevance labels (1 =
relevant, 0 = not) in *ranked order*, best-scored first. This is the standard
information-retrieval setup used by every ranking-metric guide.

These live in the installed package rather than in ``evals/`` because both the
in-CLI harness (``reporadar.evaluation``) and the offline benchmark need them, and
two copies of a metric definition drift.
"""
