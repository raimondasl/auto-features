"""Load and validate .anonymous.yml configuration."""
...
DEFAULT_CONFIG_NAME = ".anonymous.yml"
...
def _expand_env(value: Any) -> Any:
    ...
    Lets a committed ``.anonymous.yml`` reference secrets by name
...
class RankingConfig:
    ...
    # Boost (and gate) for "extends work you starred": when > 0, Anonymous fetches
...
@dataclass
class OutputConfig:
    digest_path: str = "./anonymous_digest.md"
    top_n: int = 15
...
class AzureOpenAIConfig:
    ...
    tokens come from `az login` (see anonymous.azure_auth).
    """
...
class HydeConfig:
    ...
    including every repository with no arXiv bibliography. See anonymous/hyde.py.
    """

    enabled: bool = False
    index_dir: str = "~/.cache/anonymous/hyde-index"
...
class FinescaleConfig:
    ...
    (see anonymous/finescale.py). With it off nothing changes; with it on, papers
...
# Section name -> its dataclass, read off `AnonymousConfig`'s own fields rather than
...
def _section_types() -> dict[str, type]:
    """Every `AnonymousConfig` field whose default_factory builds a config dataclass."""
    from dataclasses import MISSING
    from dataclasses import fields as dataclass_fields

    out: dict[str, type] = {}
    for f in dataclass_fields(AnonymousConfig):
...
def _dict_to_config(data: dict[str, Any]) -> AnonymousConfig:
    """Build a AnonymousConfig from a raw dict (parsed YAML)."""
...
def load_config(config_path: str | Path | None = None) -> AnonymousConfig:
    """Load config from a YAML file.

    If *config_path* is None, looks for .anonymous.yml in the current directory.
...
def _azure_warnings(cfg: AnonymousConfig) -> list[str]:
...
def validate_config(cfg: AnonymousConfig) -> list[str]:
    ...
    if "openalex" in cfg.sources and not cfg.openalex.api_key:
        warnings.append(
            ...
            "key at https://openalex.org/ and set openalex.api_key in .anonymous.yml."
        )
...
def default_config_yaml() -> str:
    """Return the default .anonymous.yml content as a string."""
    return """\
repo_path: .
...
output:
  digest_path: ./anonymous_digest.md
  top_n: 15
...
def measured_config_yaml() -> str:
    ...
    """
    return """\
# Anonymous -- the measured configuration.
#
...
#   uv pip install "anonymous-papers[hyde]"     sentence-transformers + pyarrow
...
#   gate         ~$0.05   Haiku over `triage.top_k` papers (measured $0.050 at 50)
...
#   ------------------------------------------------------------------------------
#   total        ~$0.05, measured (evals/cost_measured.json), against ~$9.50/repo at
#                list prices for the current agentic baseline, Claude Opus 5.
#
