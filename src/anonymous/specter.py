...
Scholar serves precomputed SPECTER2 vectors for free, so Anonymous gets
...
- **Vectors are cached** in the store's ``paper_embeddings`` table under a distinct
  ...
  384-dim MiniLM vectors used by :mod:`anonymous.semantic` (different keys,
...
- **Scores are pool-normalized.** Raw SPECTER2 cosines sit in a narrow band
  ...
  spread — the same trick :func:`anonymous.citations.normalize_citations` uses.
"""
...
from anonymous.embedding_cache import vec_from_bytes, vec_to_bytes
