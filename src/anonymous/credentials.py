...
(Copilot inherits PATH and nothing else). The plugin's ``.mcp.json`` is committed to version
control. VS Code's ``inputs`` prompt does not exist for plugin-bundled configs --
...
``uvx --from "anonymous-papers[mcp]==<version>" rr mcp``, so the executable path changes on
...
_ENV_CONFIG_DIR = "ANONYMOUS_CONFIG_DIR"


def config_dir() -> Path:
    """The per-user configuration directory, overridable like GH_CONFIG_DIR.

    ``~/.config/anonymous`` on macOS as well as Linux, deliberately: the GitHub CLI does the
    ...
    if sys.platform == "win32":
        ...
        if base:
            return Path(base) / "anonymous"
        return Path.home() / "AppData" / "Roaming" / "anonymous"
