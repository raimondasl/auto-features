...
from anonymous.credentials import resolve_api_key
...
_NO_KEY = {
    "openai": (
        "No OpenAI API key. Set suggestions.openai_api_key in .anonymous.yml (it may be "
        "${OPENAI_API_KEY}), export OPENAI_API_KEY, or run `rr auth` to store one."
    ),
    "claude": (
        "No Claude API key. Set suggestions.claude_api_key in .anonymous.yml (it may be "
...
def _call_openai_top_logprobs(
    ...
    *distribution* rather than the sampled score — reading the expectation over the
    ...
    (see anonymous/finescale.py and evals/RESULTS.md). Anthropic's API exposes no
...
def top_logprobs(
    ...
    *cfg* needs ``openai_api_key`` (or ``OPENAI_API_KEY`` in the environment),
    ...
    if patterns:
        from anonymous.privacy import compile_patterns, redact
