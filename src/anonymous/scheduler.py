...
TASK_NAME = "Anonymous"
CRON_MARKER = "# Anonymous scheduled run"
...
def add_cron_job(cron_expr: str, config_path: str) -> bool:
    """Add or replace a Anonymous cron job. Returns True on success."""
    command = _build_command(config_path)
    crontab = _get_current_crontab()

    # Remove existing Anonymous lines
...
def list_cron_jobs() -> list[ScheduledTask]:
    """Parse Anonymous entries from the current crontab."""
    crontab = _get_current_crontab()
    tasks = []
...
def remove_cron_job() -> bool:
    """Remove all Anonymous lines from crontab. Returns True on success."""
    crontab = _get_current_crontab()
    lines = [line for line in crontab.splitlines() if CRON_MARKER not in line]
    new_content = "\n".join(lines) + "\n" if lines else ""
    return _set_crontab(new_content)
...
def list_schtasks() -> list[ScheduledTask]:
    """List Anonymous scheduled tasks on Windows."""
    result = subprocess.run(
...
def remove_schtask() -> bool:
    """Remove the Anonymous Windows scheduled task. Returns True on success."""
    result = subprocess.run(
        ["schtasks", "/Delete", "/TN", TASK_NAME, "/F"],
        capture_output=True,
        text=True,
