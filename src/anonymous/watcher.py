    ...
    try:
        if system == "Linux":
            ...
            ps_script = (
                ...
                "CreateToastNotifier('Anonymous').Show($toast)"
            )
            result = subprocess.run(
...
def run_update_cycle(
    ...
    from anonymous.digest import categorize_papers, write_digest
    from anonymous.pipeline import LogReporter, open_store, run_pipeline
    from anonymous.profiler import cited_arxiv_ids_of
    from anonymous.stages import WATCH, unrun_stages
    from anonymous.store import StoreError
