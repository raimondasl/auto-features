...
The 0-3 triage gate (:mod:`anonymous.triage`) is near-binary in practice: almost
...
* **This path is OpenAI-only.** Reading a token distribution needs logprobs, which the
  ...
  and needs ``OPENAI_API_KEY``; with it off, Anonymous behaves exactly as before.
...
from anonymous.llm_client import LLMError, RateLimitBreaker, top_logprobs
...
def score_papers(
    ...
    *,
    ...
    :class:`~anonymous.llm_client.LLMUnavailable` instead, as :func:`triage_papers` does.
    """
