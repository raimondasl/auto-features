...
from anonymous.config import LLM_PROVIDERS
from anonymous.llm_client import LLMError, LLMUnavailable
from anonymous.paper_id import dedup_id as _dedup_id
from anonymous.profiler import profile_repo
from anonymous.provenance import source_marker
    ...
    *,
    ...
    if not queries:
        if cfg.queries.redact:
            ...
            report.warn(
                "No queries to run. Add seed queries to .anonymous.yml or ensure "
    ...
    with open_store(db_path, report) as store:
        ...
        if cfg.feedback.enabled:
            try:
                from anonymous.feedback import compute_adjusted_weights
        ...
        if cfg.ranking.hybrid:
            from anonymous.retrieval import hybrid_reorder
        ...
        try:
            from anonymous.trends import compute_keyword_frequencies
        ...
        if cfg.enrichment.provider != "off":
            try:
                from anonymous.sources.hf_papers import fetch_enrichments_batch
...
def _collect_extra_sources(
    ...
    *,
    ...
    if cfg.recommendations.enabled:
        try:
            from anonymous.sources.s2_recommendations import fetch_recommendations
...
def _merge_source(
    ...
    *,
    ...
    Each paper it adds is stamped with *source* (see `anonymous.provenance`). None of these
...
def _enrich(
    ...
    *,
    ...
    if repo_embedding is not None:
        try:
            from anonymous.embedding_cache import default_model, embed_papers_cached
    ...
    if cfg.ranking.w_citations > 0:
        ...
        try:
            from anonymous.citations import fetch_citation_counts, normalize_citations
    ...
    if cfg.ranking.w_citation_proximity > 0:
        try:
            from anonymous.citation_graph import build_seed_set, find_citation_links
            from anonymous.citations import fetch_references
    ...
    if cfg.ranking.w_specter > 0:
        try:
            from anonymous.specter import SPECTER_MODEL
            from anonymous.specter import score_papers as specter_score_papers
    ...
    if cfg.ranking.w_community > 0:
        try:
            from anonymous.sources.hf_papers import normalize_upvotes
    ...
    if cfg.signals.hackernews:
        try:
            from anonymous.signals.hn import fetch_attention, normalize_points
...
def _integrity(
    ...
    if not cfg.signals.integrity:
        return withdrawn
    try:
        from anonymous.signals.integrity import fetch_comments, find_withdrawn, stale_ids
...
def _triage(
    ...
    *,
    ...
    if cfg.triage.enabled and cfg.suggestions.provider in LLM_PROVIDERS:
        ...
        try:
            from anonymous.evidence import partition_by_evidence
