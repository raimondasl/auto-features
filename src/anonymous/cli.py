"""Click CLI entry points for Anonymous."""
...
from anonymous import credentials, finescale
...
from anonymous.pipeline import run_pipeline
from anonymous.profiler import profile_repo
from anonymous.ranker import format_score_explanation, score_distribution
from anonymous.store import PaperStore, StoreError
...
class JsonReporter:
    ...
    Anonymous's MCP server delegates collection to a `uvx` environment when the repository
    asks for dense discovery and the server's own environment cannot provide it (see
    `anonymous.delegate`). A tool call that goes silent for the minutes that takes is a tool
...
@click.version_option(package_name="anonymous-papers")
def cli() -> None:
...
def init(path: str, measured: bool) -> None:
    """Initialize Anonymous in a repository.

    Creates .anonymous.yml and the .anonymous/ storage directory.
    ...
    success("Anonymous initialized. Edit .anonymous.yml to customize.")
    ...
    if measured:
        ...
        info("     rescore is OpenAI-only either way. Published numbers were measured")
        info("     with the gate on claude-haiku-4-5; an OpenAI gate has since been")
        info("     measured too, and is a wash in the shipped digest: +0.08 net@2")
        info("     paired over 37 cases, CI [-0.86, +1.02], under both judges (NR-63).")
        info('  2. uv pip install "anonymous-papers[hyde]" && rr sync-index   # one time, ~1.1 GB')
...
@click.option(
    ...
    help="Path to .anonymous.yml (default: .anonymous.yml in current dir).",
)
...
@click.option(
    ...
    "For a parent process driving this run — Anonymous's MCP server uses it to forward a "
...
def update(
    ...
    repo_path = Path(cfg.repo_path).resolve()
    db_path = repo_path / ".anonymous" / "papers.db"
...
def digest(
    ...
    path configured in .anonymous.yml.
    """
    if verbose:
        setup_verbose_logging()
    ...
    if summary and cfg.hooks.on_digest:
        from anonymous.notify import run_shell_hook
...
def search(
    ...
    if use_semantic:
        from anonymous.embeddings import EMBEDDINGS_AVAILABLE

        if not EMBEDDINGS_AVAILABLE:
            error(
                "Semantic search needs the embeddings extra: "
                "pip install 'anonymous-papers[embeddings]'"
            )
            raise SystemExit(1)

    from anonymous.digest import filter_since
    ...
    with _open_store(db_path) as store:
        ...
        if use_semantic:
            from anonymous.embedding_cache import default_model
...
def notify(config_path: str | None, channel: str, run_id: int | None) -> None:
    ...
    from anonymous.digest import categorize_papers, find_extends_starred
    from anonymous.notify import DigestSummary, dispatch_notification
...
def status(config_path: str | None) -> None:
    """Show the current Anonymous status."""
    cfg = _load_and_validate(config_path)
    repo_path = Path(cfg.repo_path).resolve()
    db_path = repo_path / ".anonymous" / "papers.db"
...
def audit(config_path: str | None, as_json: bool, verbose: bool) -> None:
    ...
    from anonymous.privacy import REPO_AND_CONTENT, audit_plan
...
@click.option(
    "--labels",
    default="anonymous",
    help="Comma-separated labels to add (default: anonymous).",
)
@click.option("-v", "--verbose", is_flag=True, help="Enable verbose logging.")
def gh_issues(
    config_path: str | None,
    ...
    from anonymous.gh_issues import check_gh_available, create_issues
...
@click.option(
    ...
    help="Serve this store instead of <repo>/.anonymous/papers.db.",
)
def mcp(config_path: str | None, db_override: str | None) -> None:
    """Run Anonymous as an MCP server (stdio) for coding agents.
    ...
    / Windsurf.
    Requires the optional MCP extra:  uv pip install "anonymous-papers[mcp]"
    ...
    from anonymous.mcp_server import require_sdk, run_stdio
    ...
    try:
        ...
        error('The optional extra is:  uv pip install "anonymous-papers[mcp]"')
        raise SystemExit(1) from None
...
def why(arxiv_id: str, config_path: str | None, run_id: int | None, as_json: bool) -> None:
    ...
    from anonymous.digest import cited_ids_from
...
def schedule(config_path: str | None, cron: str | None, list_flag: bool, remove: bool) -> None:
    ...
    from anonymous.scheduler import add_schedule, list_schedules, remove_schedule
...
def workspace_init() -> None:
    ...
    from anonymous.workspace import ensure_workspace_dir, open_workspace_store
...
def workspace_update(verbose: bool) -> None:
    ...
    from anonymous.workspace import open_workspace_store, score_papers_for_repo
    ...
    try:
        ...
        for repo in repos:
            ...
            from anonymous.stages import WORKSPACE, drift_warning
...
def workspace_digest(run_id: int | None, output_path: str | None, fmt: str) -> None:
    ...
    from anonymous.workspace import combined_digest_data, open_workspace_store
    ...
    try:
        ...
        env = Environment(
            loader=PackageLoader("anonymous", "templates"),
...
def auth(provider: str, show_status: bool, do_remove: bool) -> None:
    ...
    could go are all closed: `.anonymous.yml` gets committed, the plugin's `.mcp.json` is under
    version control, and a value passed through an MCP tool call would land in the
...
def doctor(config_path: str | None) -> None:
    ...
    if gate in credentials.PROVIDERS:
        ...
        (
            ...
            if key
                ...
                f"suggestions.{credentials.PROVIDERS[gate][0]} in .anonymous.yml, export "
    ...
    if cfg.hyde.enabled:
        ...
        if shards < 0:
            gap(
                'hyde.enabled but the extra is missing — uv pip install "anonymous-papers[hyde]"',
    ...
    if cfg.ranking.w_embedding > 0:
        ...
        (
            ...
            if EMBEDDINGS_AVAILABLE
                ...
                'uv pip install "anonymous-papers[embeddings]" (or [hyde], which includes it).',
            )
        )
    else:
...
def sync_index(config_path: str | None, refresh: bool, verify: bool) -> None:
    ...
    try:
        ...
        hyde_cfg = HydeConfig()
        info("No .anonymous.yml here — syncing to the default location.")
...
def watch(config_path: str | None, interval: str, no_notify: bool) -> None:
    ...
    """
    from anonymous.watcher import parse_interval as _parse_interval
    from anonymous.watcher import watch_loop
    ...
    from anonymous.stages import WATCH, drift_warning
