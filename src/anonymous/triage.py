...
agentic Opus baseline showed the real gap is *precision* — Anonymous ranks many
...
from anonymous.llm_client import LLMError, RateLimitBreaker, complete
...
def repo_context_block(profile: RepoProfile, summary: Any = None) -> str:
    ...
    rescore (:mod:`anonymous.finescale`) describe a repo with the *same bytes*. That is
...
def triage_papers(
    ...
    *summary* is passed to every paper's prompt, so the one repo-summarisation call is
    ...
    Raises :class:`~anonymous.llm_client.LLMUnavailable` instead of trying the next paper when
