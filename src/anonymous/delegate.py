...
The plugin's MCP server is installed as ``anonymous-papers[mcp]``: click, pyyaml, arxiv and
...
runs as ``uvx --from "anonymous-papers[hyde]==<this version>" rr update`` instead of in
...
def installed_version() -> str | None:
    """The version of Anonymous running in this process, or None if it is not installed.
...
def uvx_executable() -> str | None:
    ...
    working directory, which is the repository: see :mod:`anonymous.executables`."""
...
def child_paths(cfg: Any, repo: Path) -> tuple[Path, Path]:
    ...
    return child_repo, (child_repo / ".anonymous" / "papers.db").resolve()
...
def index_synced(cfg: Any, repo: Path) -> bool:
    ...
    """
    from anonymous.hyde import index_shards
...
def plan(cfg: Any, *, repo: Path, config_path: Path, db: Path) -> Plan:
    ...
    if version is None:
        return Plan(
            ...
            warning=(
                "Dense discovery cannot run here, and Anonymous's own version could not be "
...
def run(plan: Plan, *, report: Any, timeout: float | None = None) -> dict[str, Any]:
    ...
    try:
        proc = subprocess.Popen(
            ...
            # the child. Found in a real VS Code session -- the child
