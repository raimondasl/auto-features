...
file runs. Anonymous's working directory is the repository being profiled -- ``rr update`` in a
clone, the delegated ``uvx`` child, an editor-launched server -- and the repository can be one the
user did not write, with a committed ``.anonymous.yml`` choosing which program gets looked up. So
