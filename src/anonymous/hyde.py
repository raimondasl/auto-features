"""HyDE discovery: search arXiv by the abstract of a paper the repository would benefit from.

An LLM writes short hypothetical abstracts for the papers that would most improve the
repository. Such text is phrased the way papers are phrased, so it can be embedded and matched
against real abstracts in a dense index. Queries built from the repository's own text describe
what the code already does, and they reach few of the papers worth adopting. Section 4 of the
paper gives the measurements, and `evals/hyde_replication.py` reproduces them.
...
from anonymous.llm_client import complete
...
USER_AGENT = f"Anonymous/{__version__} (+https://anonymous.4open.science/r/utility-judgments)"
...
def sync_index(index_dir: Path, *, refresh: bool = False, on_shard: Any = None) -> dict[str, int]:
    ...
    try:
        ...
        raise HydeError(
            'HyDE needs pyarrow and numpy. Install with: uv pip install "anonymous-papers[hyde]"'
        ) from exc
...
def load_encoder(model_name: str = MODEL_NAME) -> Any:
    try:
        ...
        raise HydeError(
            ...
            'uv pip install "anonymous-papers[hyde]"'
        ) from exc
...
def generate_hypotheses(
    ...
    **after** :func:`~anonymous.triage.repo_context_block`, never merged into it, and two
    ...
    * feeding stated wants to the **gate** was measured at net@2 +57 against +95 for the
    ...
    So a goal changes what Anonymous goes looking for and never what it accepts.
    """
