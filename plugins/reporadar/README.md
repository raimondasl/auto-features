...
```bash
uvx --from "reporadar-papers[mcp]==1.0.4" rr --version
```
...
```bash
uvx --from "reporadar-papers==1.0.4" rr auth                    # OpenAI, the default
uvx --from "reporadar-papers==1.0.4" rr auth --provider claude  # ...or Anthropic
uvx --from "reporadar-papers==1.0.4" rr auth --status           # what is stored, and from where
```
...
```bash
uvx --from "reporadar-papers[hyde]==1.0.4" rr sync-index
```
...
(`reporadar-papers[mcp]==1.0.4`) — not from `main`, and no longer from a git tag — so what you run
